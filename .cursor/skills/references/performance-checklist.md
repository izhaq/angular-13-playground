...
### Change Detection
- [ ] `ChangeDetectionStrategy.OnPush` used for presentational components
- [ ] `trackBy` function used with `*ngFor` for lists
- [ ] Observables handled with `async` pipe (avoids manual subscription management)
- [ ] No heavy computation in component templates

### Bundle Size
- [ ] Lazy loading for feature modules (`loadChildren`)
- [ ] AOT compilation enabled for production (`ng build --configuration production`)
- [ ] Bundle analyzed: `ng build --stats-json && npx webpack-bundle-analyzer dist/stats.json`
- [ ] Or use: `ng build --source-map && npx source-map-explorer dist/**/*.js`

### Runtime
- [ ] `ngZone.runOutsideAngular()` used for non-UI async operations
- [ ] `ChangeDetectorRef.detectChanges()` over `ChangeDetectorRef.markForCheck()` for immediate needs
