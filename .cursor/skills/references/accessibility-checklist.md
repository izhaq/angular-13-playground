...
Angular Material components have built-in accessibility. Key points:

- [ ] `mat-label` used inside `mat-form-field` for form labeling
- [ ] `mat-error` used for accessible error messages (announced by screen readers)
- [ ] `mat-select` preferred over custom dropdowns (built-in keyboard navigation)
- [ ] `MatDialog` used for modals (handles focus trap and restoration automatically)
- [ ] `mat-icon` buttons always have `aria-label` or `matTooltip`
- [ ] `mat-tab-group` used for tab interfaces (handles ARIA roles automatically)
- [ ] `mat-autocomplete` used instead of custom typeaheads
- [ ] `cdkTrapFocus` used for custom overlays/panels that need focus management

### Angular-Specific Testing

```bash
# Add axe-core to Karma tests
