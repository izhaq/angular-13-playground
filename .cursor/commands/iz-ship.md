...
Launch three subagents concurrently using the Task tool with `subagent_type: "generalPurpose"`. **Issue all three Task tool calls in a single assistant turn so they execute in parallel** — sequential calls defeat the purpose of this command. Give each subagent the persona definition to follow:
