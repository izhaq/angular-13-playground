...
exports.registerEngineSimRoutes = void 0;
const ws_1 = require("ws");
const state_1 = require("./state");
const ROUTE_PREFIX = '/api/engine-sim';
const WS_PATH = `${ROUTE_PREFIX}/ws`;
/**
 * Wires the Engine Sim feature endpoints onto an existing express app +
 ...
 */
function registerEngineSimRoutes(app, server) {
    ...
    wss.on('connection', (ws) => {
        clients.add(ws);
        console.log(`[engine-sim WS] connected (total: ${clients.size})`);
        ...
        ws.on('close', () => {
            clients.delete(ws);
            console.log(`[engine-sim WS] disconnected (total: ${clients.size})`);
        });
        ws.on('error', (err) => {
            console.error('[engine-sim WS] error:', err.message);
            clients.delete(ws);
        });
    });
    function broadcast() {
        ...
        console.log(`[engine-sim WS] broadcast → ${delivered} client(s)`);
    }
    app.post(`${ROUTE_PREFIX}/primary`, (req, res) => {
        ...
        console.log(`[engine-sim] POST /primary — sides=${result.payload.sides.join(',')}` +
    ...
    app.post(`${ROUTE_PREFIX}/secondary`, (req, res) => {
        ...
        console.log(`[engine-sim] POST /secondary — sides=${result.payload.sides.join(',')}` +
            ` wheels=${result.payload.wheels.join(',')}` +
            ` fields=${Object.keys(result.payload.fields).length}`);
        broadcast();
        res.json({ status: 'accepted' });
...
exports.registerEngineSimRoutes = registerEngineSimRoutes;
