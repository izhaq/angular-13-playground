...
import { EngineSimResponse } from './models';
 ...
 */
export function registerEngineSimRoutes(app: Express, server: HttpServer): void {
  const state: EngineSimResponse = buildInitialState();
  ...
  app.post(`${ROUTE_PREFIX}/primary`, (req, res) => {
    ...
    console.log(
      `[engine-sim] POST /primary — sides=${result.payload.sides.join(',')}` +
  ...
  app.post(`${ROUTE_PREFIX}/secondary`, (req, res) => {
    ...
    console.log(
      `[engine-sim] POST /secondary — sides=${result.payload.sides.join(',')}` +
      ` wheels=${result.payload.wheels.join(',')}` +
      ` fields=${Object.keys(result.payload.fields).length}`,
    );
    broadcast();
