 ...
 *
 * `EngineSimDataService` is stubbed with a manual subject so the spec
 * controls when grid frames arrive without spinning up real timers/WS.
 */
describe('EngineSimShellComponent', () => {
  ...
  let fixture: ComponentFixture<EngineSimShellComponent>;
  let component: EngineSimShellComponent;
  let apiSpy: jasmine.SpyObj<EngineSimApiService>;
  let frames$: Subject<EngineSimResponse>;

  beforeEach(async () => {
    apiSpy = jasmine.createSpyObj<EngineSimApiService>('EngineSimApiService', [
      'postPrimary',
      'postSecondary',
    ]);
    ...
    frames$ = new Subject<EngineSimResponse>();
    const dataStub: Partial<EngineSimDataService> = {
      connect: () => frames$.asObservable(),
    };

    const noopWs: EngineSimWebSocketFactory = () => new Subject<EngineSimResponse>().asObservable();
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: ENGINE_SIM_WS_FACTORY, useValue: noopWs },
        { provide: EngineSimApiService, useValue: apiSpy },
        { provide: EngineSimDataService, useValue: dataStub },
      ],
    }).compileComponents();

    fixture = TestBed.createComponent(EngineSimShellComponent);
