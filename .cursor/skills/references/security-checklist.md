...
- [ ] No unnecessary `bypassSecurityTrust*` calls in DomSanitizer usage
- [ ] Angular's template sanitization not bypassed
- [ ] HttpClient configured for XSRF protection (automatic with cookies)
- [ ] No direct `innerHTML` binding with user data (use Angular's built-in sanitization)
